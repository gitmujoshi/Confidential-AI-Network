...
/**
 * Create Fresh Test Users via APIs
 * 
 * This script creates completely fresh test users with unique emails
 * using only the public APIs. All users are created through the registration API
 * which properly handles Keycloak integration.
 * 
 * Uses centralized config.env for all configuration.
 */

const axios = require('axios');

// Load centralized configuration from config.env
const { loadConfig } = require('./scripts/load-config.js');
loadConfig();
...
// Fresh test users with unique emails
const testUsers = [
  {
    name: 'Medical Data Provider',
    email: `tdp.medical.${timestamp}@test.com`,
    partyType: 'TDP',
    organization: 'Medical Data Solutions Inc.',
    description: 'Healthcare data provider specializing in medical imaging datasets'
  },
  {
    name: 'NLP Research Lab',
    email: `tdp.nlp.${timestamp}@test.com`,
    partyType: 'TDP',
    organization: 'NLP Research Foundation',
    description: 'Natural language processing datasets for research and development'
  },
  {
    name: 'AutoDrive Data Corp',
    email: `tdp.autodrive.${timestamp}@test.com`,
    partyType: 'TDP',
  ...
  },
  {
    ...
    email: `tdc.healthcare.${timestamp}@test.com`,
    partyType: 'TDC',
...
async function createFreshTestData() {
  try {
    log('🚀 Starting Fresh Test Data Creation via APIs', 'bright');
    log('=' .repeat(60), 'cyan');
    log(`📅 Test run timestamp: ${timestamp}`, 'cyan');
    ...
    log('\n🔍 Step 2: Registering fresh test users...', 'blue');
    ...
    // Step 5: Summary
    log('\n📊 Fresh Test Data Creation Summary', 'bright');
    log('=' .repeat(40), 'cyan');
    ...
    log('\n👥 Created Users with Working Credentials:', 'blue');
    ...
    log('\n🎉 Fresh test data creation completed!', 'bright');
    log('💡 All users can login with their respective passwords', 'yellow');
    log(`📝 Test data saved with timestamp: ${timestamp}`, 'yellow');
...
module.exports = { createFreshTestData, APIHelper };
