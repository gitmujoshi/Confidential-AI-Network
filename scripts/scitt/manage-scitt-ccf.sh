...
start_scitt_ccf() {
    ...
    if docker compose version > /dev/null 2>&1; then
        run_compose "docker-compose.scitt-ccf-dev.yml" -p "$COMPOSE_PROJECT_NAME" down --remove-orphans > /dev/null 2>&1 || true
        run_compose "docker-compose.scitt-ccf-dev.yml" -p "$COMPOSE_PROJECT_NAME" up -d --remove-orphans --force-recreate
    else
        COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" run_compose "docker-compose.scitt-ccf-dev.yml" down --remove-orphans > /dev/null 2>&1 || true
        COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" run_compose "docker-compose.scitt-ccf-dev.yml" up -d --remove-orphans --force-recreate
    fi
...
show_logs() {
    ...
    if [ -f "$(compose_path "docker-compose.scitt-ccf-dev.yml")" ]; then
        ...
        run_compose "docker-compose.scitt-ccf-dev.yml" logs -f
    else
        print_warning "SCITT CCF Docker Compose file not found"
    fi
}
