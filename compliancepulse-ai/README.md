...
**Research seam with CAN:** Confidential AI Network forwards Open-GMASE decisions to `POST /api/v1/audit/ingest` by default (`COMPLIANCEPULSE_INGEST_URL=http://localhost:3001`). If this service is down, CAN logs a warning and continues. Set `COMPLIANCEPULSE_INGEST_URL=false` on CAN to disable. That is an ingest stub—not a finished multi-tenant SaaS product.
