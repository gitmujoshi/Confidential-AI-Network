...
# Convert IAM Specifications to PDF Document (Basic Method)
# This script converts the markdown IAM specifications to a PDF document using basic pandoc

echo "Converting IAM Specifications to PDF document (Basic Method)..."
...
if [ $? -eq 0 ]; then
    ...
    if [ $? -eq 0 ]; then
        echo "✅ Successfully converted to HTML: ../output/IAM_Specifications.html"
        echo "📄 You can open this HTML file in a browser and print to PDF"
        echo "   open ../output/IAM_Specifications.html"
    else
        echo "❌ Error converting document"
        exit 1
    fi
...
if [ -f "../output/IAM_Specifications.pdf" ]; then
