...
> **Published on the CAN blog:** [Beyond instance principals: fixing the pod identity gap in OKE](https://gitmujoshi.github.io/Confidential-AI-Network/security/2026/07/29/beyond-instance-principals-oke-spiffe/)
>
> This file remains the canonical copy in `docs/architecture/`.

The problem we're actually facing: we're building AI agents on OKE (Oracle Container Engine for Kubernetes), and each one needs its own scoped access to OCI resources and external APIs — without falling back on one shared credential across the whole agent fleet. That's a straightforward ask on AWS or GCP. On OCI, it means wiring SPIFFE identity through a federation path that exists but isn't built for this out of the box.
...
This isn't hypothetical anymore. OpenAI shipped Workload Identity Federation for its own API — trusted workloads exchange a cloud-issued OIDC token for a short-lived OpenAI access token, no stored API key.

When configuring this with OCI, developers typically rely on OCI Instance Principals or Resource Principals to sign a token exchange with the identity domain. However, **on OKE, the standard instance principal signer identifies the underlying worker node, not the individual pod.**

Furthermore, while OKE clusters *do* support exposing an OIDC discovery endpoint for Kubernetes Service Account projected tokens (similar to EKS or GKE Workload Identity), **OCI IAM lacks native, out-of-the-box Kubernetes Service Account to OCI IAM policy mapping**. Without custom wiring or SPIFFE, three different agent pods on the same node — each running a different task, for a different user, with different intended scope — present as the exact same identity to that federation flow.
...
One of these per agent namespace (matching the `agents-research` / `agents-finance` / `agents-orchestrator` split) gets every matching pod its own SVID automatically, with no manual registration step per pod.
...
4. Exchange an SVID for an OCI token. `POST <IAM-DOMAIN-URL>/oauth2/v1/token`, passing the agent's JWT-SVID as `subject_token`, `subject_token_type=jwt`, and `requested_token_type=urn:oci:token-type:oci-upst`. A working exchange returns a UPST, usable with OCI SDKs for up to 60 minutes.
...
**Out:** what OCI returns. The response itself is just `{"token": "<opaque-token-id>"}` — Oracle doesn't publish the full internal UPST claim schema, so this is illustrative based on documented fields:
