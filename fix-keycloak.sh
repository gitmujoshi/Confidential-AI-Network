...
run_diagnose() {
    ...
    cd backend/scripts
    node fix-***REMOVED-KEYCLOAK_DB_PASSWORD***-integration.js
    cd ../..
}
...
run_quick_fix() {
    ...
    cd backend/scripts
    node quick-***REMOVED-KEYCLOAK_DB_PASSWORD***-fix.js
    cd ../..
}
...
run_setup() {
    ...
    cd backend
    node setup-***REMOVED-KEYCLOAK_DB_PASSWORD***.js
    cd ..
}
...
check_status() {
    ...
    if curl -s http://localhost:8080/realms/master > /dev/null 2>&1; then
        ...
        echo "docker run -p 8080:8080 -e KEYCLOAK_ADMIN=admin -e KEYCLOAK_ADMIN_PASSWORD=admin quay.io/***REMOVED-KEYCLOAK_DB_PASSWORD***/***REMOVED-KEYCLOAK_DB_PASSWORD***:latest start-dev"
    fi
}
