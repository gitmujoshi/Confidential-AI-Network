...
async function testDirectKeycloakAuth() {
  ...
  console.log('===========================================');
  console.log(`URL: ${CONFIG.***REMOVED-KEYCLOAK_DB_PASSWORD***Url}/realms/${CONFIG.***REMOVED-KEYCLOAK_DB_PASSWORD***Realm}/protocol/openid-connect/token`);
  ...
  try {
    ...
    const response = await axios.post(
      `${CONFIG.***REMOVED-KEYCLOAK_DB_PASSWORD***Url}/realms/${CONFIG.***REMOVED-KEYCLOAK_DB_PASSWORD***Realm}/protocol/openid-connect/token`,
      requestBody,
      {
...
async function main() {
  ...
  try {
    ...
    const ***REMOVED-KEYCLOAK_DB_PASSWORD***Success = await testDirectKeycloakAuth();
    ...
    console.log('\n📊 Debug Summary:');
    console.log('==================');
    console.log(`🔐 Direct Keycloak Auth: ${***REMOVED-KEYCLOAK_DB_PASSWORD***Success ? '✅ SUCCESS' : '❌ FAILED'}`);
    console.log(`🌐 Backend Login API: ${backendSuccess ? '✅ SUCCESS' : '❌ FAILED'}`);
    
    if (***REMOVED-KEYCLOAK_DB_PASSWORD***Success && !backendSuccess) {
