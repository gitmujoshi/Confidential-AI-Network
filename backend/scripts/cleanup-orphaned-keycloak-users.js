...
async function main({ dryRun = true } = {}) {
  ...
  try {
    ...
    const adminToken = await ***REMOVED-KEYCLOAK_DB_PASSWORD***.getAdminToken();
    let ***REMOVED-KEYCLOAK_DB_PASSWORD***Users = [];
    let first = 0;
    const max = 100;
    let fetched = 0;
    do {
      const response = await require('axios').get(
        `${***REMOVED-KEYCLOAK_DB_PASSWORD***.baseURL}/admin/realms/${***REMOVED-KEYCLOAK_DB_PASSWORD***.realm}/users?first=${first}&max=${max}`,
        { headers: { Authorization: `Bearer ${adminToken}` } }
      );
      fetched = response.data.length;
      ***REMOVED-KEYCLOAK_DB_PASSWORD***Users = ***REMOVED-KEYCLOAK_DB_PASSWORD***Users.concat(response.data);
      first += max;
    } while (fetched === max);
    ...
    for (const u of orphans) {
      try {
        await ***REMOVED-KEYCLOAK_DB_PASSWORD***.deleteUser(u.id);
