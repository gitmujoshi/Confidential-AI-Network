...
class QuickKeycloakFix {
  ...
  async runSetupScript() {
    ...
    try {
      const setupScript = path.join(this.backendPath, 'setup-keycloak.js');
  ...
  async runQuickFix() {
    ...
    if (!isConnected) {
      ...
      this.log('docker run -p 8080:8080 -e KEYCLOAK_ADMIN=admin -e KEYCLOAK_ADMIN_PASSWORD=admin quay.io/keycloak/keycloak:latest start-dev', 'fix');
      return;
    }
    ...
    this.log('   node backend/scripts/fix-keycloak-integration.js', 'fix');
  }
}
