...
async function createUsers() {
  ...
  for (const userData of testUsers) {
    try {
      ...
      const keycloakUserResponse = await axios.post(`${KEYCLOAK_BASE_URL}/admin/realms/${KEYCLOAK_REALM}/users`, {
      ...
      const keycloakUserId = keycloakUserResponse.headers.location.split('/').pop();
      ...
      await axios.post(
        `${KEYCLOAK_BASE_URL}/admin/realms/${KEYCLOAK_REALM}/users/${keycloakUserId}/role-mappings/realm`,
        [roleResponse.data],
        {
          headers: {
            'Authorization': `Bearer ${adminToken}`,
