...
async function checkPrerequisites() {
  ...
  try {
    ...
    const ***REMOVED-KEYCLOAK_DB_PASSWORD***Response = await axios.get(`${KEYCLOAK_BASE_URL}/health`, { timeout: 5000 });
    console.log('✅ Keycloak is running');
  } catch (error) {
    console.error('❌ Keycloak is not running. Please start Keycloak first.');
    console.log('💡 Start Keycloak with: docker-compose up ***REMOVED-KEYCLOAK_DB_PASSWORD***');
    process.exit(1);
  }
