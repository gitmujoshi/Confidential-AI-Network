...
class KeycloakDiagnostic {
    ...
    try {
      ...
      if (error.response?.status === 404) {
        ...
        this.issues.push({
          ...
          fix: 'Create the realm using setup-***REMOVED-KEYCLOAK_DB_PASSWORD***.js'
        });
      } else {
    ...
    try {
      ...
      if (response.data && response.data.length > 0) {
        ...
        this.issues.push({
          ...
          fix: 'Create the client using setup-***REMOVED-KEYCLOAK_DB_PASSWORD***.js'
        });
        return null;
      }
    } catch (error) {
  ...
  async checkRolesExist() {
    ...
    if (missingRoles.length > 0) {
      this.issues.push({
        ...
        fix: 'Create missing roles using setup-***REMOVED-KEYCLOAK_DB_PASSWORD***.js'
      });
    }
  ...
  generateReport() {
    ...
    if (this.issues.length === 0) {
      ...
      this.log('1. Run: node backend/setup-***REMOVED-KEYCLOAK_DB_PASSWORD***.js', 'fix');
    ...
    for (const issue of this.issues) {
      if (issue.type === 'realm' || issue.type === 'client' || issue.type === 'roles') {
        ...
        try {
          ...
          const setupPath = path.join(__dirname, '..', 'setup-***REMOVED-KEYCLOAK_DB_PASSWORD***.js');
  ...
  diagnostic.runDiagnostics()
    .then(async () => {
      if (diagnostic.issues.length > 0) {
        ...
        console.log('\n📋 To apply fixes, run: node backend/setup-***REMOVED-KEYCLOAK_DB_PASSWORD***.js');
      }
    })
    .catch((error) => {
