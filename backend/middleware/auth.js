...
const authenticateToken = async (req, res, next) => {
  try {
    ...
    try {
      ...
      const ***REMOVED-KEYCLOAK_DB_PASSWORD***HealthCheck = await axios.get(`${process.env.KEYCLOAK_URL || 'http://localhost:8080'}/realms/${process.env.KEYCLOAK_REALM || 'contract-management'}/protocol/openid-connect/certs`, { timeout: 2000 });
      
      if (***REMOVED-KEYCLOAK_DB_PASSWORD***HealthCheck.status === 200) {
