...
// Helper function to get existing TDP users with admin token
async function getExistingTDPUsers(adminToken) {
  try {
    console.log('🔍 Getting existing TDP users...');
    ...
    if (response.data.success) {
      const tdpUsers = response.data.users.filter(user => user.partyType === 'TDP' && user.isActive);
      console.log(`✅ Found ${tdpUsers.length} active TDP users`);
      return tdpUsers;
    } else {
    ...
    console.log('❌ Error getting users:', error.response?.data?.error || error.message);
    return [];
  }
}
...
async function createDatasetsAndModels() {
  ...
  // Get existing TDP users
  const tdpUsers = await getExistingTDPUsers(adminToken);
  
  if (tdpUsers.length === 0) {
    console.log('❌ No TDP users found. Please create TDP users first.');
    return;
  }
