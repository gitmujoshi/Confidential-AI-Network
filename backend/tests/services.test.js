...
describe('Service Layer Test Suite', () => {
  ...
  describe('Keycloak Service', () => {
    describe('User Management', () => {
      it('should create user in Keycloak', async () => {
        ...
        const result = await keycloakService.createUser(testUser);

        expect(result.success).toBe(true);
        expect(result.userId).toBe(mockKeycloakUser.id);
        expect(result.user).toEqual(mockKeycloakUser);
        expect(keycloakService.createUser).toHaveBeenCalledWith(testUser);
      });
      ...
      it('should handle Keycloak user creation failure', async () => {
        ...
        await expect(keycloakService.createUser(testUser))
      ...
      it('should update user in Keycloak', async () => {
        ...
        keycloakService.updateUser = jest.fn().mockResolvedValue({
          success: true,
          message: 'User updated successfully'
        });

        const result = await keycloakService.updateUser(testUser.id, updateData);

        expect(result.success).toBe(true);
        expect(result.message).toBe('User updated successfully');
        expect(keycloakService.updateUser).toHaveBeenCalledWith(testUser.id, updateData);
      });

      it('should delete user from Keycloak', async () => {
        keycloakService.deleteUser = jest.fn().mockResolvedValue({
          success: true,
          message: 'User deleted successfully'
        });

        const result = await keycloakService.deleteUser(testUser.id);

        expect(result.success).toBe(true);
        expect(result.message).toBe('User deleted successfully');
        expect(keycloakService.deleteUser).toHaveBeenCalledWith(testUser.id);
      });
    });

    describe('Authentication', () => {
      it('should authenticate user with Keycloak', async () => {
        const mockToken = 'keycloak-access-token';
        const mockUserInfo = {
        ...
        keycloakService.authenticateUser = jest.fn().mockResolvedValue({
          success: true,
          token: mockToken,
          userInfo: mockUserInfo
        });

        const result = await keycloakService.authenticateUser('test@example.com', 'password');

        expect(result.success).toBe(true);
        expect(result.token).toBe(mockToken);
        expect(result.userInfo).toEqual(mockUserInfo);
        expect(keycloakService.authenticateUser).toHaveBeenCalledWith('test@example.com', 'password');
      });

      it('should handle authentication failure', async () => {
        keycloakService.authenticateUser = jest.fn().mockRejectedValue(
          new Error('Invalid credentials')
        );

        await expect(keycloakService.authenticateUser('test@example.com', 'wrongpassword'))
          .rejects.toThrow('Invalid credentials');
      });

      it('should validate Keycloak token', async () => {
        const mockToken = 'valid-keycloak-token';

        keycloakService.validateToken = jest.fn().mockResolvedValue({
          success: true,
          valid: true,
          userInfo: {
            sub: 'keycloak-user-id',
        ...
        const result = await keycloakService.validateToken(mockToken);

        expect(result.success).toBe(true);
        expect(result.valid).toBe(true);
        expect(result.userInfo).toBeDefined();
        expect(keycloakService.validateToken).toHaveBeenCalledWith(mockToken);
      });
    });

    describe('Role Management', () => {
      it('should assign role to user', async () => {
        keycloakService.assignRole = jest.fn().mockResolvedValue({
          success: true,
          message: 'Role assigned successfully'
        });

        const result = await keycloakService.assignRole(testUser.id, 'TDP');

        expect(result.success).toBe(true);
        expect(result.message).toBe('Role assigned successfully');
        expect(keycloakService.assignRole).toHaveBeenCalledWith(testUser.id, 'TDP');
      });

      it('should get user roles', async () => {
        const mockRoles = ['TDP', 'user'];

        keycloakService.getUserRoles = jest.fn().mockResolvedValue({
          success: true,
          roles: mockRoles
        });

        const result = await keycloakService.getUserRoles(testUser.id);

        expect(result.success).toBe(true);
        expect(result.roles).toEqual(mockRoles);
        expect(keycloakService.getUserRoles).toHaveBeenCalledWith(testUser.id);
      });
    });
  });
  ...
  describe('Service Integration', () => {
    it('should handle service dependencies correctly', async () => {
      ...
      expect(keycloakService.createUser).not.toHaveBeenCalled(); // Already exists
