 ...
 * - Hybrid Mode: Smart Contract + SCITT CCF integration
 */
...
describe('Contract State Machine Tests', () => {
  ...
  describe('Hybrid Mode Tests (Smart Contract + SCITT CCF)', () => {
    let hybridContract;
    ...
    beforeAll(async () => {
      ...
      try {
        ...
        console.warn('SCITT CCF service not available for hybrid tests:', error.message);
      }
    });

    it('should create contract in both systems in hybrid mode', async () => {
      const contractData = {
        contractId: 'HYBRID-TEST-001',
        title: 'Hybrid Test Contract',
        description: 'Contract for hybrid mode testing',
        price: 500.00,
        duration: 60,
        termsAndConditions: 'Hybrid test terms',
        contractDatasets: [{
          datasetId: testDataset.id,
          tdpId: tdpUser.id,
          datasetName: testDataset.name,
      ...
      hybridContract = await contractService.createContract(contractData, tdcUser.id);
      expect(hybridContract.status).toBe('DRAFT');

      // Submit to SCITT CCF if available
      if (scittCcfService.isInitialized) {
        try {
          const scittResult = await scittCcfService.createContract({
            name: hybridContract.title,
            description: hybridContract.description,
            tdpId: tdpUser.id,
            tdcId: tdcUser.id,
            datasetId: testDataset.id,
            price: hybridContract.price,
            duration: hybridContract.duration,
            terms: hybridContract.termsAndConditions
          });

          expect(scittResult.success).toBe(true);
          expect(scittResult.source).toBe('SCITT_CCF');
          expect(scittResult.claimId).toBeDefined();

          // Store SCITT CCF claim ID
          hybridContract.scittClaimId = scittResult.claimId;
        } catch (error) {
          console.warn('SCITT CCF contract creation failed:', error.message);
        }
      }
    });

    it('should handle state transitions in hybrid mode', async () => {
      if (!hybridContract) {
        throw new Error('Hybrid contract not created');
      }

      // Submit contract
      await contractService.submitContract(hybridContract.contractId, tdcUser.id);
      let updatedContract = await contractService.getContract(hybridContract.contractId);
      expect(updatedContract.status).toBe('PENDING_TDP');

      // TDP signs
      await contractService.tdpSignContract(hybridContract.contractId, tdpUser.id, {});
      ...
      // Start execution
      await contractService.startExecution(hybridContract.contractId);
      updatedContract = await contractService.getContract(hybridContract.contractId);
      expect(updatedContract.status).toBe('EXECUTING');

      // Complete execution
      await contractService.completeExecution(hybridContract.contractId);
      updatedContract = await contractService.getContract(hybridContract.contractId);
      expect(updatedContract.status).toBe('COMPLETED');
    });

    it('should verify SCITT CCF integration in hybrid mode', async () => {
      if (!hybridContract?.scittClaimId || !scittCcfService.isInitialized) {
        console.log('Skipping SCITT CCF verification - not available');
        return;
      }

      // Get SCITT CCF contract status
      const scittStatus = await scittCcfService.getContractStatus(hybridContract.scittClaimId);
      expect(scittStatus).toHaveProperty('status');
      expect(scittStatus).toHaveProperty('claimId', hybridContract.scittClaimId);

      // Verify provenance tracking
      const provenanceTree = await scittCcfService.getProvenanceTree(hybridContract.scittClaimId);
    ...
    it('should handle partial system failures gracefully', async () => {
      ...
      // Test blockchain failure with SCITT CCF fallback
      if (scittCcfService.isInitialized) {
        // Mock blockchain failure
        const originalCreateContract = contractService.createContract;
        contractService.createContract = jest.fn().mockRejectedValue(new Error('Blockchain unavailable'));

        try {
          // Should still work with SCITT CCF
          const fallbackResult = await scittCcfService.createContract({
            name: 'Fallback Contract',
            description: 'Contract created after blockchain failure',
            tdpId: tdpUser.id,
            tdcId: tdcUser.id,
            datasetId: testDataset.id,
            price: 200.00,
          ...
          expect(fallbackResult.source).toBe('SCITT_CCF');
        } finally {
          // Restore original function
          contractService.createContract = originalCreateContract;
        }
      }
    });

    afterAll(async () => {
      // Cleanup hybrid contract
      if (hybridContract) {
        try {
          await hybridContract.destroy();
        } catch (error) {
          console.warn('Failed to cleanup hybrid contract:', error.message);
        }
      }
    });
  });
