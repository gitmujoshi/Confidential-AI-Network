...
describe('Mock Integration Test Suite', () => {
  ...
  async function createTestDataset() {
    // Ensure tdpUser exists before creating dataset
    if (!tdpUser || !tdpUser.id) {
      throw new Error('TDP user not created yet');
    }
    ...
    console.log('🧪 Creating mock test dataset with data:', JSON.stringify(datasetData, null, 2));
    console.log('🧪 Mock TDP User:', JSON.stringify(tdpUser, null, 2));
