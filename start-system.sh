...
docker-compose -f docker-compose.dev.yml up -d ***REMOVED-DB_PASSWORD***-app
...
if [ -f "backend/setup-***REMOVED-KEYCLOAK_DB_PASSWORD***-persistent.sh" ]; then
    chmod +x backend/setup-***REMOVED-KEYCLOAK_DB_PASSWORD***-persistent.sh
    cd backend
    ./setup-***REMOVED-KEYCLOAK_DB_PASSWORD***-persistent.sh --skip-user-sync || echo "⚠️  Keycloak setup had minor issues but continuing..."
    cd ..
else
