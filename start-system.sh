...
fix_database_connection() {
    ...
    if docker exec postgres-cms psql -U postgres -c "SELECT 1;" > /dev/null 2>&1; then
    ...
    docker exec postgres-cms psql -U postgres -c "CREATE USER postgres WITH PASSWORD 'postgres';" 2>/dev/null || true
    docker exec postgres-cms psql -U postgres -c "ALTER USER postgres WITH SUPERUSER;" 2>/dev/null || true
    docker exec postgres-cms psql -U postgres -c "CREATE DATABASE contract_management OWNER postgres;" 2>/dev/null || true
